"""Download public HIGGS parquet splits and materialize a local HIGGS.pt cache."""
...
def combine_splits(split_names, urls, download_dir, *, prefix, force_download=False):
    ...
    for split in split_names:
        ...
        parquet_path = download_dir / f"{prefix}_{split}.parquet"
        ensure_download(urls[split], parquet_path, force=force_download)
...
def main():
    ...
    if not args.keep_parquet:
        for split in [*args.train_splits, *args.test_splits]:
            parquet_path = data_dir / f"{slug}_{'train' if split in args.train_splits else 'test'}_{split}.parquet"
