...
def full_training_pipeline(H=32, B=8, K_EMBED=784, num_cols=8):
    ...
    loss_ty    = np.ndarray[(1,),                 np.dtype[np.float32]]
    ...
    head_fwd_k   = Kernel("head_forward_loss_bf16", "full_training_kernels.a",
                          [act_ty, head_wt_ty, labels_ty, d_logits_ty, loss_ty])
    ...
    loss_fifo   = ObjectFifo(loss_ty,   name="loss",   depth=1)

    # ── Gradient chain (backward) ──────────────────────────────────
    grad_fifos = [ObjectFifo(act_ty, name=f"grad_{i}", depth=1)
                  for i in range(NUM_RESIDUAL + 1)]

    # ── Re-stream input for embed backward ─────────────────────────
    embed_in_bwd = ObjectFifo(embed_in_ty, name="embed_in_bwd", depth=1)
    ...
    def make_embed_worker():
        def w(of_x_in, of_y_out, of_wt, of_x_bwd, of_grad_in,
              fwd_k, bwd_k):
            x = of_x_in.acquire(1)
    ...
    workers.append(Worker(
        ...
        [embed_in.cons(), act_fifos[0].prod(), embed_wt_cons,
         embed_in_bwd.cons(), grad_fifos[NUM_RESIDUAL].cons(),
    ...
    def make_head():
        def w(of_yin, of_wt, of_lab, of_loss,
              of_cp, of_cr, of_dlp, of_dlc, of_gout,
              cp_k, fwd_k, bwd_k):
            y = of_yin.acquire(1)
            wt = of_wt.acquire(1)
            lb = of_lab.acquire(1)
            lo = of_loss.acquire(1)
            ck = of_cp.acquire(1)
            dl = of_dlp.acquire(1)
    ...
    host_labels_ty   = np.ndarray[(B,),                    np.dtype[np.int32]]
    host_loss_ty     = np.ndarray[(1,),                    np.dtype[np.float32]]

    rt = Runtime()
    with rt.sequence(
        host_embed_in_ty, host_embed_wt_ty, host_res_wt_ty,
        host_head_wt_ty, host_labels_ty, host_loss_ty,
        host_embed_in_ty,
    ) as (x_raw, wt_embed, wt_res, wt_head, labels, loss_out, x_raw_bwd):
        rt.start(*workers)
        ...
        rt.fill(labels_fifo.prod(), labels, task_group=tg_fwd)
        rt.drain(loss_fifo.cons(),  loss_out, wait=True, task_group=tg_fwd)
        rt.finish_task_group(tg_fwd)

        tg_bwd = rt.task_group()
        rt.fill(embed_in_bwd.prod(), x_raw_bwd, task_group=tg_bwd)
